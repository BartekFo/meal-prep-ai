<script lang="ts">
	...
	const totalTime = $derived(`${recipe.prepTime} min prep · ${recipe.cookTime} min cook`);
...
<Card class="gap-0 overflow-hidden">
	...
	<CardContent class="space-y-6">
		<div class="grid grid-cols-2 gap-4 sm:grid-cols-3">
			...
			<RecipeStatCard icon={Users} label="Servings" value={recipe.servings} />
	...
	{#if toolState === 'output-available'}
		<CardFooter class="flex flex-col gap-2 pt-4">
			...
			{:else}
				<Button class="w-full" disabled={isLoading} onclick={handleAddRecipe} type="button">
					{#if isLoading}
